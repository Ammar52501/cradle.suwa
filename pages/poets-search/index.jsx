...
const poetsSearch = ({ erasAllEras, dataDefault, translations }) => {
  ...
  const handleChange = (event) => {
    ...
    if (selectedValue === 0) {
      // If no era is selected (i.e., 'all' option), show all poets
      filteredData = dataDefault;
    } else if (selectedValue) {
      // If a specific era is selected, filter poets by their 'zamanId'
      filteredData = dataDefault.filter(poet => poet.zamanId === selectedValue);
    } else {
      // Fallback in case no era is selected (for edge cases)
      filteredData = dataDefault;
    }
  ...
  return (
    <>
      <Head>
        <title>{translations.explorePoets}</title>
        <meta charset="UTF-8" />
              ...
              <div className={styles.slider_container}>
                  ...
                  <div className={styles.filter_methods}>
                    ...
                    <div className={styles.box}>
                      <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                        ...
                        >
                          <MenuItem value={0} sx={menuItemStyle}>{translations.allages}</MenuItem>
                          <MenuItem value={true} sx={menuItemStyle}>{translations.poetswholivedintheKingdomofSaudiArabia}</MenuItem>
                          {erasAllEras?.map((era) => (
                ...
                {filtredPoets?.length === 0 &&
                  <Box sx={{ textAlign: 'center' }}>
                    <h1>{translations.noResults} {searchString}</h1>
                  </Box>
                }
              </div>

              <div className={styles.text_container}><p>
                {translations.drawDesc}
              </p></div>
